...
.nyan-player-wrapper
    ...
    .nyan-player(:class='{ "nyan-player-mini": isMinimize, "nyan-player-auto-hidden": isAutoHidden && isMinimize, "nyan-player--playlist-hidden": isHidePlayList || isMinimize }')
        ...
        .nyan-player__cover(v-if='currentMusic && currentMusic.cover' @click='displayStatus && isMinimize ? displayStatus.paused ? onResume() : onPause() : 0' :data-playing="displayStatus && !displayStatus.paused")
            img.nyan-player__cover(:src='currentMusic.cover' alt='')
        ...
        .nyan-player__status
            .nyan-player__info
                .nyan-player__info-title(:data-content="currentMusic.title") {{ currentMusic.title || 'None' }}
                .nyan-player__info-artist(:data-content="currentMusic.artist") {{ currentMusic.artist || '' }}
        ...
        ul.nyan-player__playlist(ref='playlist')
            li(v-for='(music, index) in musics' @click='playMusicByIndex(index)' :class='{ active: currentMusic.id == music.id }' :key="index") 
...
export default defineComponent({
    ...
    computed: {
        ...
        shuffleIds() {
            return this.shuffle(this.musics.map(music => music.id))
        },
    },
    watch: {
        volumeValue(nv, ov) {
    ...
    methods: {
        ...
        playListControl(skip, shuffleIds) {
            if (shuffleIds) {
                const currentIndex = this.shuffleIds.findIndex(id => id == this.currentMusic.id)
                const nextIndex = currentIndex + skip
                const loopIndex = nextIndex < 0 ? shuffleIds.length - 1 : nextIndex > shuffleIds.length - 1 ? 0 : nextIndex
                this.playMusicByIndex(this.musics.findIndex(music => music.id == shuffleIds[loopIndex]))
                return
            }
            const currentIndex = this.musics.findIndex(item => item.id == this.currentMusic.id)
            this.playMusicByIndex(currentIndex + skip)
        },
        async playMusicByIndex(index, isAutoPlay = true) {
            index = index < 0 ? this.musics.length - 1 : index > this.musics.length - 1 ? 0 : index
            this.currentMusic = this.musics[index]
            // this.currentMusic = this.mus ics[Math.min(Math.max(index, 0), this.musics.length - 1)]
            // this.currentMusic = this.musics[index < 0 ? 0 : index > this.musics.length - 1 ? this.musics.length - 1 : index]
            if (isAutoPlay && !this.config.paused) this.$nextTick(() => this.audio.play())

            const realIndex = this.musics.findIndex(item => item.id == this.currentMusic.id)
        ...
        onPlayControl(skip = 1) {
            ...
            this._timer = setTimeout(() => this._timer = undefined, 500);
            switch (this.playMode) {
                case this.PLAYMODE_RANDOM:
                    this.playListControl(skip, this.shuffleIds)
                    break;
