export default {
    ...
    app: {
        ...
        hitokoto: [
            ...
            {
                content: '白鳥は　かなしからずや　空の青　海のあをにも　染まずただよふ',
                from: '西園 美魚',
            },
            {
                content: '人的生命就是一场梦，而梦是用璀璨夺目的“魔法”做成的。将它献给樱花树的话，樱花一定会盛开吧。',
                from: 'さくら、もゆ',
            },
        ],
        nav: [
            { name: 'Home', icon: 'home', to: '/blog/', theme: { color: '#cd5da0' } },
            { name: 'Blog', icon: 'blog', to: '/blog/posts/1', theme: { color: '#cd5da0' } },
            { name: 'Project', icon: 'project', to: '/blog/projects', theme: { color: '#f2c047' } },
            { name: 'Idea', icon: 'idea', to: '/blog/idea', theme: { color: '#00ccff' } },
            // { name: 'Experience', icon: 'experience', to: '/experience' },
            // { name: 'Timeline', icon: 'event_note', to: '/timeline', theme: { color: '#8daacd' } },
            { name: 'About', icon: 'about', to: '/blog/about', theme: { color: '#c7b3d6' } },
        ...
        musicRequestMethod: async () => {
            try {
                ...
                const response = await (await fetch(
                    `${musicApi}/playlist/detail?id=${musicListId}`, { method: 'GET', mode: 'cors' })).json()
                const ids = response.playlist.trackIds.map(item => item.id)
                const result = await (await fetch(
                    `${musicApi}/song/detail?ids=${ids.join(',')}`)).json()
                // 按以下接口返回
                // interface Music {
                //     id: number
                //     title: string
